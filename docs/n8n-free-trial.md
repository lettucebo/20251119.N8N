...
- **繁體中文｜ZH**：
	1. 準備可收信的公司或團隊電子郵件，以便接收註冊確認與 magic link。
	2. 預先決定工作區（Workspace）名稱、部署區域與主要管理者資訊，確保符合組織命名及資料主權需求。
	3. 確認您能接受 n8n 服務條款與隱私政策，並在需要時先通過公司資安／法遵審核。
	4. 準備至少一個可被自動化的流程範例（自有或官方 Template），以便開通後立即測試。
- **English｜EN**:
	1. Have a reachable work/team email inbox ready for the signup confirmation and magic-link messages.
	2. Decide on your workspace name, preferred region, and primary admin details ahead of time to satisfy naming and data-residency policies.
	3. Ensure you can accept the n8n Terms of Service and Privacy Policy, including any internal security/compliance approvals you might need.
	4. Collect at least one workflow scenario (your own or a template) so you can validate value as soon as the workspace is live.

## 申請流程 / Signup Steps

![Screenshot Placeholder – Step 1](./images/placeholder-step1.png)

- **繁體中文｜ZH**：前往 [n8n Cloud](https://n8n.io/pricing/) 並點選「Start free trial」或「Try for free」，輸入您的姓名與工作信箱後提交即可收到建立工作區的連結。
- **English｜EN**: Visit [n8n Cloud](https://n8n.io/pricing/) and click "Start free trial"/"Try for free". Provide your name plus work email, then submit to receive the workspace-creation link in your inbox.

![Screenshot Placeholder – Step 2](./images/placeholder-step2.png)

- **繁體中文｜ZH**：在建立精靈中設定 Workspace 名稱、地區與首位管理者帳號，選擇安全密碼、勾選條款，最後點擊「Create workspace」即可觸發部署。
...
- **繁體中文｜ZH**：收到確認信後，使用信中的 magic link 啟用帳號並自動登入工作區；首次登入可依需求載入官方模板或匯入既有 workflow JSON。
...
- **繁體中文｜ZH**：若部署完成後無法從 app 進入，可再次登入並選擇「Send magic link」以透過郵件安全開啟 Admin Dashboard，進入後建議立即設定時區與通知，並於 Settings 中啟用 2FA 或 SSO 以保護試用環境。(來源：[Cloud admin dashboard](https://docs.n8n.io/manage-cloud/cloud-admin-dashboard/))
...
- **繁體中文｜ZH**：透過 Admin Dashboard 可檢視版本、啟用/停用工作流程並設定時區；若前端無法登入，重新請求 magic link 仍可直接存取。同時建議定期在「Executions」中檢查 1,000 次配額使用狀況，並透過「Upgrade」提早轉正，或利用 [Download workflows](https://docs.n8n.io/manage-cloud/download-workflows/) 將自動化備份至本地以防到期刪除。(來源：[Cloud admin dashboard](https://docs.n8n.io/manage-cloud/cloud-admin-dashboard/))
...
- **繁體中文｜ZH**：在登入頁選擇「Send magic link」，從郵件開啟即可繞過前端故障直接進入 Dashboard，適用於主站維護或網路異常時。(來源：[Cloud admin dashboard](https://docs.n8n.io/manage-cloud/cloud-admin-dashboard/))
